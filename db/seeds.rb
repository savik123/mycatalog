...
Equipment.find_each do |e|
  e.update!(
    application: case e.name
                 when /3D|Photon|Creality|Bambu|CreatBot/
                   "Применяется для прототипирования, создания моделей и деталей в образовательных и инженерных проектах."
                 when /станок|Lathe|Milling|Metal|JET/
                   "Используется для обработки металлических заготовок в лабораторных и производственных условиях."
                 when /принтер|Epson|HP|Kyocera/
                   "Применяется для печати документов и графических материалов в офисах и учебных заведениях."
                 when /двигатель/
                   "Используется как привод в различных механических и лабораторных установках."
                 else
                   "Используется в учебных и лабораторных работах для выполнения профильных задач."
                 end,

    recommendations: case e.name
                     when /3D/
                       "Перед началом работы выполнить калибровку, использовать качественные материалы и соблюдать температурный режим."
                     when /станок|Lathe|Milling/
                       "Соблюдать технику безопасности, использовать защитные средства и регулярно обслуживать оборудование."
                     when /принтер/
                       "Использовать оригинальные расходные материалы и не допускать перегрузки устройства."
                     else
                       "Соблюдать инструкцию по эксплуатации и проводить регулярное техническое обслуживание."
                     end
  )
end

puts "🧠 DESCRIPTIONS ADDED"
# =========================
